...
case class NoResultFoundError(override val period: IPeriod, override val target: ITermTarget, val articleDescr: String, val contractOpt: Option[ContractCode] = None, val positionOpt: Option[PositionCode] = None)
  extends TermResultError(period, target, s"result for ${articleDescr}${TermResultError.messageContractPosition(contractOpt, positionOpt)} Not Found")
case class NullResultFoundError(override val period: IPeriod, override val target: ITermTarget, val articleDescr: String, val contractOpt: Option[ContractCode] = None, val positionOpt: Option[PositionCode] = None)
  extends TermResultError(period, target, s"result found for ${articleDescr}${TermResultError.messageContractPosition(contractOpt, positionOpt)} but Instance is Null!")